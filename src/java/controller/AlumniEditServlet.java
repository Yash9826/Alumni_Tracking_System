...
public class AlumniEditServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            Alumni alumni = new Alumni(id, name, email, enrollment, registration, year, occupation, organisation, designation, dateOfBirth, password, contact, img.getSubmittedFileName());
            AlumniDAO dao = new AlumniDAO(ConnectionProvider.getConnection());
           // System.out.println(alumni.toString());

            if (dao.updateAlumni(alumni)) {
                response.sendRedirect("alumniLogin.jsp");
//                 out.println("done");
            } else {
                response.sendRedirect("alumniEdit.jsp");
                out.println("error");
            }
